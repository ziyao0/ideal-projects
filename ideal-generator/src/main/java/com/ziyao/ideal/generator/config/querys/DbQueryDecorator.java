...
public class DbQueryDecorator extends AbstractDbQuery {
    ...
    public DbQueryDecorator(@NonNull DataSourceConfig dataSourceConfig, @NonNull StrategyConfig strategyConfig) {
    ...
    public static class ResultSetWrapper {
        ...
        private String getComment(String columnLabel) {
            return Strings.hasLength(columnLabel) ? formatComment(getStringResult(columnLabel)) : Strings.EMPTY;
        }
        ...
        public boolean isPrimaryKey() {
            ...
            if (DbType.DB2 == dbType || DbType.SQLITE == dbType || DbType.CLICK_HOUSE == dbType) {
                return Strings.hasLength(key) && "1".equals(key);
            } else {
                return Strings.hasLength(key) && "PRI".equalsIgnoreCase(key);
            }
        }
    }
}
