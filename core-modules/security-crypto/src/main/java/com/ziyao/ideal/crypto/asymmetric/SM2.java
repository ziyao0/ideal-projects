...
import com.ziyao.ideal.crypto.utils.BCUtils;
import com.ziyao.ideal.crypto.utils.ECKeyUtils;
 ...
 * 参考：<li><a href="https://blog.csdn.net/pridas/article/details/86118774">国密算法签名</a></li>
 *
 * <p>
 * 国密算法包括：
 * <ol>
...
public class SM2 extends AbstractAsymmetricCrypto<SM2> {
    ...
    public SM2(String privateKeyStr, String publicKeyStr) {
        this(SecureUtils.decode(privateKeyStr), SecureUtils.decode(publicKeyStr));
    }
    ...
    public SM2(byte[] privateKey, byte[] publicKey) {
        this(
                ECKeyUtils.decodePrivateKeyParams(privateKey),
                ECKeyUtils.decodePublicKeyParams(publicKey)
        );
    }
     ...
     * * 在硬件签名中，返回结果为R+S，可以通过调用{@link SmUtils#rsAsn1ToPlain(byte[])}方法转换。
     *
     * @param data 加密数据
     * @return 签名
     */
    ...
    /**
     * 用私钥对信息生成数字签名，签名格式为ASN1<br>
     * 在硬件签名中，返回结果为R+S，可以通过调用{@link SmUtils#rsAsn1ToPlain(byte[])}方法转换之。
     *
     * @param data 被签名的数据数据
